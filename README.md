...
- **Top up**: ask Claude Code to "add N more days to quotes.json". Append-only — never reorder, so past days keep their quote. Entries without translations publish with English titles/descriptions only; translations can be added to existing entries later (safe — it doesn't reorder).
