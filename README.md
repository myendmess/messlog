...
- **Localization runway**: only the translated stretch of the bank reaches the 12 markets — after it, Shorts ship English-only while the run stays green. `pick_quote.py` warns 30 days out and every day past the end, so the cliff shows up in the run summary instead of silently shrinking reach.
...
- The description carries a subscribe nudge above the music credit. This was an owner comment posted after each upload, but the Data API cannot pin comments and an unpinned comment on a Short goes unseen — so it needed manual pinning in Studio every day, and the track links it carried were already in the description. In the description it is always visible and needs nothing.
